...
async def start_session_handler(request: Request, data: SessionStart, db: Session = Depends(get_db)):
    try:
        session = session_crud.create_session(data, db)
        return session
    except IntegrityError:
        db.rollback()
        raise HTTPException(status_code=409, detail="Session for this user on this date already exists")
...
async def stop_session_handler(request: Request, data: SessionStop, db: Session = Depends(get_db)):
    try:
...
@limiter.limit("5/minute;50/hour")
async def admin_adjust_session_handler(request: Request, data: SessionAdminAdjustment, db: Session = Depends(get_db)):
    try:
        ...
        raise HTTPException(status_code=500, detail="Failed to adjust session")


@router.get("/user/{user_id}", response_model=List[SessionRead])
@limiter.limit("100/minute;100/hour")
async def get_user_sessions_handler(request: Request, user_id: int, db: Session = Depends(get_db)):
    try:
...
@limiter.limit("100/minute;100/hour")
async def get_current_session_handler(request: Request, user_id: int, db: Session = Depends(get_db)):
    try:
        session = session_crud.get_current_session(user_id, db)
        return session
    except Exception as e:
        raise HTTPException(status_code=500, detail="Failed to retrieve current session")